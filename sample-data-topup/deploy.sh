#!/usr/bin/env bash
# Builds the Lambda jar and deploys sample-data-topup to AWS via Terraform, wired up to target the
# mootmaker-api deployment of the given environment name in the sibling checkout (see the
# mootmaker project README for the multi-environment how-to). Also creates a weekly EventBridge
# schedule that invokes it automatically (see deploy/terraform/schedule.tf).
...
echo "Deploying sample-data-topup, targeting mootmaker-api environment '${environment}'..."
