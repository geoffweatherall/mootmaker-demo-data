 ...
 *
 * <p>Deliberately duplicated from sample-data-generator's identical-in-spirit copy rather than
 * shared (see {@link GraphQlClient}'s doc comment for why), with one real difference: this
 * version takes an explicit {@code List<LocalDate>} of days to generate for, rather than a
 * contiguous day-offset range - {@link SampleDataTopUp} has already worked out exactly which
 * business days are empty and need topping up, which need not be contiguous (e.g. a single empty
 * weekday sandwiched between two already-populated ones).
 *
 * <p>The same extra touches as sample-data-generator keep the generated data looking like a real
 * calendar rather than a packed schedule: each room's first meeting of the day starts at a random
