variable "aws_region" {
  description = "AWS region to deploy the sample data top-up Lambda into."
  type        = string
  default     = "us-east-1"
}
...
variable "project_name" {
  ...
  type        = string
  default     = "mootmaker-sample-data-topup"
}
...
variable "schedule_expression" {
  description = "EventBridge schedule expression controlling how often this tool runs automatically. Default is every Monday at 06:00 UTC - early enough in the week to top up before people start looking at the calendar, but see the README for why the exact day/time doesn't matter much."
  type        = string
  default     = "cron(0 6 ? * MON *)"
}

variable "schedule_enabled" {
  description = "Whether the EventBridge schedule is active. Set to false to temporarily stop automatic weekly runs without destroying the schedule resource itself - ./run.sh still works either way."
  type        = bool
  default     = true
}
