...
class GraphQlClient {
   ...
   * secret are exchanged at the Cognito token endpoint for a JWT access token, so no human user or
   * password is involved. Credentials come from SSM Parameter Store at runtime (see {@link
   * SsmSecrets}) rather than from environment variables - it used to borrow the acceptance tests'
   * client, whose secret was passed in as a plaintext Lambda environment variable.
   *
  ...
  private static String fetchAccessToken(final SsmSecrets.Credentials credentials) {
    ...
    final String form =
        "grant_type=client_credentials"
            + "&client_id="
            + urlEncode(credentials.clientId())
            + "&client_secret="
            + urlEncode(credentials.clientSecret())
            + "&scope="
            + urlEncode(credentials.scope());
    ...
    try (HttpClient client = HttpClient.newHttpClient()) {
      ...
      if (response.statusCode() != 200) {
        throw new IllegalStateException(
            "Cognito token endpoint returned HTTP "
                + response.statusCode()
                + ": "
                + response.body());
      }
      ...
      if (accessToken == null) {
        throw new IllegalStateException(
            "Cognito token response contained no access_token: " + response.body());
      }
