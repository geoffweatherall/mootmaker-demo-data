 ...
 * 900-second ceiling.
 *
 * <p>The payload carries only the three concern toggles ({@code {"people": false}}), all defaulting
 * to true, so the schedule's empty payload runs everything. Magnitudes - how many people and rooms,
 * how wide the window - come from environment variables set by Terraform, so no invocation can ask
 * for more data than the deployment allows.
 */
public final class DemoDataHandler
