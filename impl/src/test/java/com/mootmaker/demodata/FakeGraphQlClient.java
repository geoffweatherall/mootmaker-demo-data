 ...
 * {@link DemoData} actually issues are understood; anything else fails loudly rather than silently
...
final class FakeGraphQlClient extends GraphQlClient {
    ...
    private final List<BulkCall> createMeetingsCalls = Collections.synchronizedList(new ArrayList<>());
    ...
    JsonNode execute(final String query, final Map<String, Object> variables) {
        ...
        if (query.contains("DatesWithMeetings")) {
            ...
            for (final Object date : (List<?>) variables.get("dates")) {
                ...
                days.add(OBJECT_MAPPER.createObjectNode().put("date", date.toString()).set("meetings", meetings));
            }
        ...
        if (query.contains("createMeetings")) {
            ...
            createMeetingsCalls.add(new BulkCall(date, batch.stream().map(FakeGraphQlClient::subjectOf).toList()));
