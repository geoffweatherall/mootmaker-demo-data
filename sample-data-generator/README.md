...
1. Calls the API's `reset` mutation. This deletes all rooms and meetings, and every person **except** those linked to a real Cognito account (e.g. anyone who's actually signed up through the webapp, and the e2e test user, are left alone — see the [API README](https://github.com/geoffweatherall/mootmaker-api#reset-and-real-user-accounts)).
