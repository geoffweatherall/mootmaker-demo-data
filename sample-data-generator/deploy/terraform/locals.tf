...
locals {
  ...
  # database-reset's function name (in ../mootmaker-api), computed the same deterministic way its
  # own Terraform names it (environment + project_name default "mootmaker-database-reset") rather
  # than looked up via that project's Terraform state - this project stays a standalone Terraform
  # root with no cross-project state coupling. That coupling crosses a repository boundary. This
  # tool invokes that Lambda as the first step of every run (see DatabaseResetInvoker), so
  # database-reset must be deployed for this environment first.
