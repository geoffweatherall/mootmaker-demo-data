...
locals {
  ...
  # mootmaker-tools/database-reset's function name, computed the same deterministic way its own
  # run.sh does (environment + its project_name default "mootmaker-database-reset") rather than
  # looked up via that project's Terraform state - this project stays a standalone Terraform root
  # with no cross-project state coupling, the same reasoning the other *_table_name variables in
  # variables.tf document. This tool invokes that Lambda as the first step of every run (see
