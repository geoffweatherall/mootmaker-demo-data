#!/usr/bin/env bash
# Invokes the deployed sample-data-generator Lambda for the given environment (see deploy.sh) -
# it resets the target mootmaker-api environment and populates it with sample data (40 people,
# 10 rooms, meetings across every business day from a week ago to seven weeks ahead).
...
function_name="${environment}-mootmaker-sample-data-generator"

echo "Invoking ${function_name} in ${AWS_REGION} (this resets and repopulates '${environment}')..."
