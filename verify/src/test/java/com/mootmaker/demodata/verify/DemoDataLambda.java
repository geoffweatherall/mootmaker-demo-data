...
final class DemoDataLambda {
  ...
  /**
   * The function's own ceiling is 900 seconds, so every client-side timeout has to clear that or a
   * legitimately long seed gets reported as a failure while the Lambda keeps running and completes
   * regardless. The SDK's defaults (and the AWS CLI's) are far shorter, which is exactly the trap
  ...
  /**
   * Clears the environment, via mootmaker-api's database-reset. Demo-data itself has no reset path
   * by design, so the suite has to reach for the api's - which is the same two-step sequence a
    ...
    try (LambdaClient lambda = client()) {
      ...
      try {
        ...
        throw new IllegalStateException(
            "Could not parse " + functionName + " response: " + body, e);
      }
    }
  }
  ...
  private static String requireEnv(final String name) {
    ...
    if (value == null || value.isBlank()) {
      throw new IllegalStateException(
          name + " is required. Run these tests via ./verify.sh <environment>.");
    }
    return value;
  }
}
