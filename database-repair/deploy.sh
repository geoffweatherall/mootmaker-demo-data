#!/usr/bin/env bash
# Builds the Lambda jar and deploys database-repair to AWS via Terraform, wired up to target the
...
echo "Deploying database-repair, targeting mootmaker-api environment '${environment}'..."
...
# Populates COGNITO_USER_POOL_ID, PEOPLE_TABLE_NAME, MEETINGS_TABLE_NAME and
...
terraform -chdir=deploy/terraform init -backend-config=backend.hcl -backend-config="key=${environment}/mootmaker-tools-database-repair/terraform.tfstate"
