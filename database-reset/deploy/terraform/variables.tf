variable "aws_region" {
  description = "AWS region to deploy the database-reset Lambda into."
  type        = string
  default     = "us-east-1"
}
...
variable "project_name" {
  ...
  type        = string
  default     = "mootmaker-database-reset"
}
...
# from the table names here plus this account/region, rather than needing the actual ARNs -
# mootmaker-api's Terraform outputs only expose names, since within its own state it never needs
# the ARNs spelled out explicitly.

variable "rooms_table_name" {
  description = "DynamoDB table name for Room records in the target environment."
  type        = string
  default     = ""
}
